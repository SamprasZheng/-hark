...
def run_council(
    ...
    *,
    ...
    """Run the debate. ``brief`` is the day's data digest (markdown ok)."""
    brief = (brief or "").strip()[:brief_cap]
    res = CouncilResult(topic=topic, model=model, backend=backend)
    if not council:
    ...
    for p in council:
        ...
        text, ok = ask_fn(p.system_prompt, user, 240)
        res.votes.append(Vote(persona=p.name, title=p.title, stance=text if ok else "(本地模型無回應)"))

    # Round 2 — 質疑 + 投票
    stance_block = "\n".join(f"[{v.title}] {v.stance}" for v in res.votes)
    by_name = {v.persona: v for v in res.votes}
    for p in council:
        others = "\n".join(f"[{v.title}] {v.stance}" for v in res.votes if v.persona != p.name)
        user = (
            f"今日簡報:\n{brief}\n\n其他分析師的立場:\n{others}\n\n"
        ...
        text, ok = ask_fn(p.system_prompt, user, 280)
        v = by_name[p.name]
        v.rebuttal_vote = text if ok else "(本地模型無回應)"
    ...
    votes_block = "\n".join(
        f"[{v.title}] 投票={v.vote} 信心={v.conviction} 動作={v.action or '—'}" for v in res.votes
    )
    user = (
        f"今日簡報:\n{brief}\n\n議會票數:多={res.tally['多']} 空={res.tally['空']} "
        f"中性={res.tally['中性']} 平均信心={res.tally['avg_conviction']}\n各人最終意見:\n{votes_block}\n\n"
        f"【任務】你是主席,用繁體中文 4-6 句做出今日結論:(1) 綜合票數的整體傾向;"
        f"(2) 點出最大分歧;(3) 一句具體可執行的觀察/紀律提醒。只建議、不下單,不要捏造數字。"
    )
    conclusion, ok = ask_fn(chair_sys, user, 420)
    res.conclusion = conclusion if ok else "(主席結論:本地模型無回應)"
    res.ok = ok and any(v.stance and "無回應" not in v.stance for v in res.votes)
    return res
...
def run_council_local(
    ...
    *,
    ...
    """Convenience: load personas and run the council on the local model."""
    ...
    ask_fn, backend = make_local_ask(model)
    res = run_council(topic, brief, ask_fn=ask_fn, backend=backend, model=model,
