...
def render_md(ctx: dict) -> str:
    ...
    L += ["", "## 📌 觀察重點 + 紀律", "",
          ...
          "- **紀律**:NVDA 是風向球也是你 [redacted]% 曝險;此報研究非加碼訊號。RSU 照排程賣到 50%,BEAR 觸發看雲廠 capex / ASIC 推論佔比。",
...
def render_discord(ctx: dict) -> str:
    ...
    P += [f"**📅 行事曆** {cal}",
          f"🧭 {ctx['regime']} ｜ 紀律:NVDA=風向球也是你[redacted]%曝險,RSU照排程賣到50%,別追高。"]
    return "\n".join(P)
...
<h2>📌 觀察重點 + 紀律</h2><div class="box">環境:<b>{ctx['regime']}</b><br>紀律:NVDA 是風向球也是你 [redacted]% 曝險;研究非加碼訊號。RSU 照排程賣到 50%,BEAR 觸發看雲廠 capex / ASIC 推論佔比。</div>
